...
using Plots, Distributed, Random, LinearAlgebra, gmres, Peaks
export surrogate_function,plot_surrogate_func,root_solve

# Function we want to find the roots of 
function surrogate_function(alpha_0,alpha_1,P0,xi,cellsA,gMemSlfN,gMemSlfA,chi_inv_coeff,P,s)
    dim = cellsA[1]*cellsA[2]*cellsA[3]*3
    # RND vector
    # s = Vector{ComplexF64}(undef,dim)
    # rand!(s)
    # Approximate solve since no direct solve for operators
...
function plot_surrogate_func(alpha_0,alpha_1,P0,cellsA,gMemSlfN,gMemSlfA,chi_inv_coeff,P,max_val,s,ind)
    xi_vals = LinRange(-max_val,max_val,max_val) 
    ...
    for i in eachindex(xi_vals)
        surr_func_vals[i] = surrogate_function(alpha_0,alpha_1,P0,xi_vals[i],cellsA,
            gMemSlfN,gMemSlfA,chi_inv_coeff,P,s)
    end
    ...
    if ind == 1
        ...
        savefig("/home/nic-molesky_lab/Pad---Ritz-and-Harmonic-Ritz-duality-checker/Pade_Ritz_HarmonicRitz_algorithms/Combining Padé, Ritz and harmonic Ritz/Operator version/Graphs/zoomed_operator1_graph_surr_func_vs_xi.png")
    
    elseif ind == 2
        plot(xi_vals, surr_func_vals, title = "Zoomed-in graph: Surrogate function vs xi",)  
        xlabel!("Xi")
        ylabel!("Surrogate function")
        savefig("/home/nic-molesky_lab/Pad---Ritz-and-Harmonic-Ritz-duality-checker/Pade_Ritz_HarmonicRitz_algorithms/Combining Padé, Ritz and harmonic Ritz/Operator version/Graphs/zoomed_operator2_graph_surr_func_vs_xi.png")
    end
    ...
    savefig("/home/nic-molesky_lab/Pad---Ritz-and-Harmonic-Ritz-duality-checker/Pade_Ritz_HarmonicRitz_algorithms/Combining Padé, Ritz and harmonic Ritz/Operator version/Graphs/minimas_graph_surr_func_vs_xi.png")
...
function first_sampling(alpha_0,alpha_1,P0,x_start,cellsA,gMemSlfN,gMemSlfA,
    ...
    if ys[end] <= 0
        while ys[end] <=0
            ...
            push!(ys,surrogate_function(alpha_0,alpha_1,P0,
                x_value,cellsA,gMemSlfN,gMemSlfA,chi_inv_coeff,P,s))
        end
    ...
    x_value = 1.5*x_value
    push!(xs, x_value)
    push!(ys, surrogate_function(alpha_0,alpha_1,P0,x_value,cellsA,gMemSlfN,
        gMemSlfA,chi_inv_coeff,P,s))
    ...
    print("xi_val_peaks first_sampling 1 ", peaks, "\n")

    if length(peaks) == 0
        x_negative = -x_value
        pushfirst!(xs, x_negative)
        pushfirst!(ys, surrogate_function(alpha_0,alpha_1,P0,x_negative,cellsA,gMemSlfN,
            gMemSlfA,chi_inv_coeff,P,s))
        print("xs", xs, "\n")
        print("ys", ys, "\n")
        # Use the padé to see if peaks are predicted in the area that is supposed to be convexity
...
function root_solve(alpha_0,alpha_1,P0,x_start,cellsA,gMemSlfN,gMemSlfA,
    ...
    print("xi_val_peaks root_solve 1 ", peaks, "\n")

    if length(peaks) == 0
        x_negative = -pade_x[end]
        pushfirst!(x_sampled, x_negative)
        pushfirst!(y_sampled, surrogate_function(alpha_0,alpha_1,P0,x_negative,cellsA,gMemSlfN,
            gMemSlfA,chi_inv_coeff,P,s))
        print("xs", xs, "\n")
        print("ys", ys, "\n")
        # Use the padé to see if peaks are predicted in the area that is supposed to be convexity
    ...
    #     x_negative = -x_value
    #     push!(xs, x_value)
    #     push!(ys, surrogate_function(alpha_0,alpha_1,P0,x_negative,cellsA,gMemSlfN,
    #         gMemSlfA,chi_inv_coeff,P,s))
    #     print("xs", xs, "\n")
    #     print("ys", ys, "\n")
    #     pks, vals = findmaxima(-y_pade)
