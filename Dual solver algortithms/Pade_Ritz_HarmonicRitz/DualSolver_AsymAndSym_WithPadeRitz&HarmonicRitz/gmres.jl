...
function GMRES_with_restart(xi,l, b, cellsA, gMemSlfN,gMemSlfA, chi_inv_coeff, P, m=20) # add cellsB for the BA case 
    n = length(b)
    ...
    for j in 1:m
        ...
        # Direct sum 
        v = xi*asym_vect(gMemSlfN,gMemSlfA, cellsA, chi_inv_coeff, P, Q[:,j])+l[1]*sym_vect(gMemSlfN,gMemSlfA,cellsA, chi_inv_coeff, P, Q[:,j])
        # Function to do the sum
        # v = sym_and_asym_sum(xi,l,gMemSlfN,gMemSlfA, cellsA, chi_inv_coeff, P, Q[:,j])	
        ...
        # Need to compare the output of these two ways of doing this product
        # Direct sum 
        value = xi*asym_vect(gMemSlfN,gMemSlfA, cellsA, chi_inv_coeff, P, x)+l[1]*sym_vect(gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff, P, x)
        # Function to do the sum
        # value = sym_and_asym_sum(xi,l,gMemSlfN,gMemSlfA, cellsA, chi_inv_coeff, P, x)
