...
export Gv_AA, GAdjv_AA, Gv_AB, A, asym, sym, asym_vect, sym_vect, 
	sym_and_asym_sum, green_vect_prod_pade # , test
...
# Function for the product used in the test Padé, Ritz and harmonic Ritz code 
function green_vect_prod_pade(alpha_0,alpha_1,xi,P0,gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff,P,vec)
	# print("size(vect)", size(vect), "\n")
	# chi_inv_coeff_dag = conj(chi_inv_coeff)
	term_1 = (alpha_1+xi)*P0*vec # *P 
	"""
	There will be eventually a second term here since P_0 will not be the 
	identity matrix but an operator.
	"""
	# term_2 = chi_inv_coeff*vec # *P # supposed to be *conj.(transpose(P)) but gives error, so let's use *P for now
	term_3 = GAdjv_AA(gMemSlfA, cellsA, vec).*(alpha_0/2) # P*
	term_4 = Gv_AA(gMemSlfN, cellsA, vec).*(alpha_0/2) # P*
	# print("size(term_1)", size(term_1), "\n")
	# print("size(term_2)", size(term_2), "\n")
	# print("term_3 " , term_3, "\n")
	return term_1.+term_3.+term_4  # .-term_2
end
