module Davidson_Operator_HarmonicRitz_module_May16
...
function jacDavRitzHarm_basic(gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff,P,alpha_coeff,
	innerLoopDim::Integer,dims::Integer,tol_MGS::Float64,tol_conv::Float64,
		tol_eigval::Float64, tol_bicgstab::Float64,solver_method)::Tuple{Float64,Float64,Float64}
	...
	for itr in 2 : innerLoopDim  # Need to determine when this for loops stops 
		...
		if solver_method == "bicgstab"
			...
			srcBasis[:, itr],nb_it = bicgstab_matrix(gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff,
			P,alpha_coeff, theta, hRitzTrg, hRitzSrc, prjCoeff, resVec, tol_bicgstab)
		...
		gramSchmidtHarm!(trgBasis, srcBasis, bCoeffs1, bCoeffs2, gMemSlfN,
			gMemSlfA,cellsA,chi_inv_coeff,P,alpha_coeff,itr, tol_MGS)
...
function jacDavRitzHarm_basic_for_restart(gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff,P,alpha_coeff,
	innerLoopDim::Integer,dims::Integer,tol_MGS::Float64,tol_conv::Float64,
...
# 	vecDim::Integer, repDim::Integer, innerLoopDim::Integer,tol_MGS::Float64,tol_conv::Float64)::Float64
	...
	for itr in 2 : innerLoopDim  # Need to determine when this for loops stops 
		...
		# Calculate Jacobi-Davidson direction
		if solver_method == "bicgstab"
			print("Start of bicgstab in basic for restart \n ")
			srcBasis[:, itr], nb_it = bicgstab_matrix(gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff,
			P,alpha_coeff, theta, hRitzTrg, hRitzSrc, prjCoeff, resVec, tol_bicgstab)
			print("End of bicgstab in basic \n ")
			nb_it_total_bicgstab_solve += nb_it
		# We can't really do a direct solve here since the Green function 
		# is an operator and not a matrix.
		# elseif solver_method == "direct"
...
function jacDavRitzHarm_restart(gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff,
	P,alpha_coeff,innerLoopDim::Integer,restartDim::Integer,dims::Integer,
		tol_MGS::Float64,tol_conv::Float64,tol_eigval::Float64, 
			tol_bicgstab::Float64,solver_method)::Tuple{Float64,Float64,Float64} # ::Float64
	...
	for it in 1:2000 # restartDim # Need to think this over 
		...
		if it == 1
			...
			theta,srcBasis,trgBasis,kMat,resVec,hRitzSrc,hRitzTrg,eigenvalues,eigenvectors,nb_it_basic_for_restart,nb_it_bicgstab_solve=
				jacDavRitzHarm_basic_for_restart(gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff,P,alpha_coeff,
		...
		gramSchmidtHarm!(restart_trgBasis, restart_srcBasis, bCoeffs1, bCoeffs2, gMemSlfN,
					gMemSlfA,cellsA,chi_inv_coeff,P,alpha_coeff,restartDim, tol_MGS)
...
function bicgstab_matrix(gMemSlfN,gMemSlfA,cellsA,chi_inv_coeff,
	P,alpha_coeff, theta, hTrg, hSrc, prjC, b, tol_bicgstab)
	# dim = size(A)[1]
