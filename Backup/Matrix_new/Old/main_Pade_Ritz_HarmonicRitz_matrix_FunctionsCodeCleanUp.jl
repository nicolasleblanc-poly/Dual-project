module main_Pade_Ritz_HarmonicRitz_matrix_FunctionsCodeCleanUp
...
function eigval_test_calcs(alpha_0,alpha_1,xi,ritz_eigval_restart,D,P1,
    ...
    (ritz_eigval_restart_test,ritz_eigvec_restart_test,harmonic_ritz_eigval_restart_test,
        ritz_eigvec_restart_test)=eigval_solve(alpha_0,alpha_1,xi_test,D,P1,innerLoopDim,
...
function xi_update(alpha_0,alpha_1,innerLoopDim,restartDim,tol_MGS,
    tol_conv,tol_eigval,tol_bicgstab,xi,D,P1,nb_max_it)
    
    """
    Solve for extremum and closest to 0 eigvals for initial xi value (often 0)
    """
    # Extreme and closest to 0 eigenvalue solve
    (ritz_eigval_restart,ritz_eigvec_restart,harmonic_ritz_eigval_restart,
        ritz_eigvec_restart)=eigval_solve(alpha_0,alpha_1,xi,D,P1,innerLoopDim,
            restartDim,tol_MGS,tol_conv,tol_eigval,tol_bicgstab)
                ...
                if harmonic_ritz_eigval_restart < 0
                    ...
                    for it = 1:nb_max_it
                        if harmonic_ritz_eigval_restart > 0 
                            ...
                            # Test to see what happens when we shift xi (and therefore 
                            # the eigenvalues by.)
                            (ritz_eigval_restart_test,harmonic_ritz_eigval_restart_test,eigval_shift)=eigval_test_calcs(alpha_0,alpha_1,ritz_eigval_restart,D,P1,
                            innerLoopDim,restartDim,tol_MGS,tol_conv,tol_eigval,tol_bicgstab)
